...
function starfall_script.init()
	if storage.apm_starfall == nil then storage.apm_starfall = {} end
	if storage.apm_starfall.event_queue == nil then storage.apm_starfall.event_queue = {} end
	if storage.apm_starfall.start_impacts == nil then
		storage.apm_starfall.start_impacts = not settings.global['apm_starfall_compat_start_impacts'].value
	end
	if storage.apm_starfall.near_impact_counter == nil then storage.apm_starfall.near_impact_counter = 1 end
	if storage.apm_starfall.surfaces == nil then storage.apm_starfall.surfaces = {} end
	if storage.apm_starfall.surfaces_borders == nil then storage.apm_starfall.surfaces_borders = {} end
end
...
function starfall_script.mod_setting_changed()
	...
	if on_min_tick ~= new_on_min_tick then
		log('Changed: apm_starfall_event_min_minutes from: "' ..
		tostring(on_min_tick) .. '" to "' .. tostring(new_on_min_tick))
		on_min_tick = new_on_min_tick
	end

	if on_max_tick ~= new_on_max_tick then
		log('Changed: apm_starfall_event_min_minutes from: "' ..
		tostring(on_max_tick) .. '" to "' .. tostring(new_on_max_tick))
...
local function calculate_borders(surface_index)
	...
	local x = storage.apm_starfall.surfaces_borders[surface.index].x / 32
	local new_x = 0
	local y = storage.apm_starfall.surfaces_borders[surface.index].y / 32
	local new_y = 0
	...
	if math.sqrt(new_x) - 1 > x then
		...
		storage.apm_starfall.surfaces_borders[surface.index].x = new_border_x
		log('Set outer border x to: ' ..
		tostring(new_border_x) .. ' tiles range for surface: "' .. tostring(surface.index) .. '"')
	end

	if math.sqrt(new_y) - 1 > y then
		local new_border_y = (math.sqrt(new_y) - 1) * 32
		storage.apm_starfall.surfaces_borders[surface.index].y = new_border_y
		log('Set outer border y to: ' ..
		tostring(new_border_y) .. ' tiles range for surface: "' .. tostring(surface.index) .. '"')
	end
end
...
local function add_surface(index)
	...
	if storage.apm_starfall.surfaces[index] then return false end
	storage.apm_starfall.surfaces[index] = true
	storage.apm_starfall.surfaces_borders[index] = clone(area_border_outer)
...
local function remove_event_by_surface_index(index)
	for i = #storage.apm_starfall.event_queue, 1, -1 do
		local event = storage.apm_starfall.event_queue[i]
		if event.surface_index == index then
			table.remove(storage.apm_starfall.event_queue, i)
...
local function remove_surface(index)
	if not storage.apm_starfall.surfaces[index] then return false end
	storage.apm_starfall.surfaces[index] = nil
	storage.apm_starfall.surfaces_borders[index] = nil
...
local function get_border(surface)
	...
	local outer_border = storage.apm_starfall.surfaces_borders[surface.index]

	if width ~= 0 and math.pow(outer_border.x, 2) > math.pow(width, 2) then
		storage.apm_starfall.surfaces_borders[surface.index].x = width
	...
	if height ~= 0 and math.pow(outer_border.y, 2) > math.pow(height, 2) then
		storage.apm_starfall.surfaces_borders[surface.index].y = height
	...
	return { outer = storage.apm_starfall.surfaces_borders[surface.index], inner = inner_border }
end
...
local function get_random_position(surface, range)
	...
	if math.random() <= 0.05 * storage.apm_starfall.near_impact_counter then
		border_outer_x = border_outer_x / 2
		border_outer_y = border_outer_y / 2
		storage.apm_starfall.near_impact_counter = 1
	else
		storage.apm_starfall.near_impact_counter = storage.apm_starfall.near_impact_counter + 1
	end
...
local function check_if_position_is_valid(surface, position)
	...
	local tiles = surface.find_tiles_filtered({ position = position, radius = resource_patch_size_max + 2, name =
	water_tiles })
	if next(tiles) ~= nil then
		return false
	end

	local resources = surface.find_entities_filtered({ position = position, radius = resource_patch_size_max + 2, type =
	'resource' })
			...
			for _, player in pairs(force.connected_players) do
				if player.surface.name == surface.name then
					for _, position in pairs(positions) do
						local marker = surface.create_entity({ name = "apm_starfall_target_explosion", position =
						position })
						player.add_custom_alert(marker, { type = "item", name = "apm_starfall_target_marker" },
							{ "", "Expected meteorite impact zone." }, true)
					end
					if settings.get_player_settings(player)['apm_starfall_event_player_alert_sound'].value then
						local setting_volume = settings.get_player_settings(player)
						['apm_starfall_event_player_sound_volume'].value
...
local function get_amount(x, y, size, richness_bonus)
	...
	local high_value = x + y
	local amount = round(
	math.random(amount_min, amount_max) * (size * 2 + 1 - high_value) / (size * 2) * richness_multiplikator *
	richness_bonus, 0)
	...
	for y = -patch_size, patch_size do
		for x = -patch_size, patch_size do
			...
			if position.x == position.x + x and position.y == position.y + y then
				surface.create_entity({ name = ore_name, position = spawn_position, amount = spawn_amount, force =
				'neutral' })
			else
	...
	if mark_impact_on_map then
		...
		for _, force in pairs(forces) do
			if force.name ~= 'enemy' and force.name ~= 'neutral' then
				...
				local tag = { icon = { type = 'item', name = 'apm_starfall_target_marker' }, position = position, text =
				text }
...
local function create_impact(surface, positions)
	...
	for _, position in pairs(positions) do
		...
		if storage.apm_starfall.start_impacts == false then
	...
	storage.apm_starfall.start_impacts = true
end

-- Function -------------------------------------------------------------------
-- storage.apm_starfall.event_queue a table of events
...
local function event_queue_add(surface, surface_index, amount, event_time, range)
	local event = { state = 0, surface = surface, surface_index = surface_index, amount = amount, positions = {}, time =
	event_time, range = range }
	table.insert(storage.apm_starfall.event_queue, event)
	log('Info: event_queue_add(): for surface: "' ..
	tostring(surface.name) .. '" with index: "' .. tostring(surface_index) ..
	'" at tick: "' .. tostring(event_time) .. '"')
end
...
local function determine_next_event()
	for surface_index, _ in pairs(storage.apm_starfall.surfaces) do
		local surface = game.surfaces[surface_index]
		local range
...
local function generate_resource_lists()
	if not resources_list then
		...
		for _, resource in pairs(apm.lib.utils.prototypes.all()) do
...
function starfall_script.on_tick()
	...
	if game.tick <= 180 or #storage.apm_starfall.surfaces == 0 then
		return
	end

	if not storage.apm_starfall.start_impacts and next(storage.apm_starfall.event_queue) == nil then
		determine_start_event()
		return
	end

	if next(storage.apm_starfall.event_queue) == nil then
...
local function remote_list_surfaces()
	if not storage.apm_starfall.surfaces[index] then return {} end
	return storage.apm_starfall.surfaces
end
