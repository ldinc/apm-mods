...
function solar_script.init()
	if storage.solar == nil then storage.solar = {} end
	if storage.solar.event_queue == nil then storage.solar.event_queue = {} end
	if storage.solar.surfaces == nil then storage.solar.surfaces = {} end
end
...
local function add_surface(index)
	...
	if storage.solar.surfaces[index] then return false end
	storage.solar.surfaces[index] = true
...
local function remove_event_by_surface_index(index)
	for i = #storage.solar.event_queue, 1, -1 do
		local event = storage.solar.event_queue[i]
		if event.surface_index == index then
			table.remove(storage.solar.event_queue, i)
		end
	end
end
...
local function remove_surface(index)
	if not storage.solar.surfaces[index] then return false end
	storage.solar.surfaces[index] = nil
...
-- Function -------------------------------------------------------------------
-- storage.solar.event_queue a table of events
...
local function event_queue_add(surface, surface_index, type, time, duration)
	...
	local event = { state = 0, surface = surface, surface_index = surface_index, type = type, time = time, duration =
	duration }
	table.insert(storage.solar.event_queue, event)

	log('Info: event_queue_add(): for surface: "' ..
	tostring(surface.name) ..
	'" with index: "' ..
	tostring(surface_index) .. '" at tick: "' .. tostring(time) .. '" added event: "' .. tostring(type_name) .. '"')
end
...
function solar_script.on_tick()
	if game.tick <= 180 or #storage.solar.surfaces == 0 then
		return
	end
