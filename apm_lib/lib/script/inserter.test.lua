...
-- 3b. fuel inventory empty (still some energy left): takes up to 5 coal from the machine it
-- feeds, straight into its own fuel slot, and leaves the hand free
do
	local pickup, drop = Machine({}), Machine({ { "coal", "normal", 10 } })
	local ins = Inserter({}, nil, pickup, drop)
	run(ins)
	expect("leech drop: 5 coal in the inserter's fuel", ins.fuel_inv.count_of("coal", "normal"), 5)
	expect("leech drop: 5 coal left in the machine", drop.fuel_inv.count_of("coal", "normal"), 5)
	expect("leech drop: hand stays empty", ins.held_stack.valid_for_read, false)
end

-- 3c. never takes the last fuel item of the machine
do
	local pickup, drop = Machine({}), Machine({ { "coal", "normal", 1 } })
	local ins = Inserter({}, nil, pickup, drop)
	run(ins)
	expect("leech last item: stays in the machine", drop.fuel_inv.count_of("coal", "normal"), 1)
end

-- 3d. hand stuck away from the pickup (no energy to move back) and no fuel: still refuels,
-- but nothing is put into the hand
do
	local pickup, drop = Machine({ { "coal", "normal", 10 } }), Machine({})
	local ins = Inserter({}, defines.entity_status.no_fuel, pickup, drop)
	ins.held_stack_position = { x = 1, y = 0 }
	run(ins)
	expect("stuck hand: refuelled", ins.fuel_inv.get_item_count() > 0, true)
	expect("stuck hand: nothing in the hand", ins.held_stack.valid_for_read, false)
end

-- 3e. hand away from the pickup with fuel: left alone (no chaining mid-swing)
do
	local pickup, drop = Machine({ { "coal", "uncommon", 10 } }), Machine({})
	local ins = Inserter({ { "coal", "normal", 1 } }, nil, pickup, drop)
	ins.held_stack_position = { x = 1, y = 0 }
	run(ins)
	expect("moving hand: no chaining", pickup.fuel_inv.count_of("coal", "uncommon"), 10)
end
