...
function apm.lib.utils.character.crafting_category.add(name, crafting_category)
	...
	if character then
		...
		for _, existing in pairs(character.crafting_categories) do
			if existing == crafting_category then
				return
			end
		end

		table.insert(character.crafting_categories, crafting_category)
	end
