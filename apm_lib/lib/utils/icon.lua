	...
	for _, prototype in pairs(prototype_list) do
		...
		if object then
			if object.icon then
				...
				return { { icon = object.icon, icon_size = icon_size } }
			end
...
function apm.lib.utils.icon.merge(icon_tables)
	...
	for _, icon in pairs(icon_tables) do
		if type(icon) == 'table' then
			...
			if icon.icon_size and min_size > icon.icon_size then
				min_size = icon.icon_size
			end
		end
	end
	...
	for _, t_icon in pairs(icon_tables) do
		if type(t_icon) == 'table' then
			...
			local scale = min_size / t_icon.icon_size
			if scale ~= 1.0 then
				t_icon.scale = scale
			end
...
function apm.lib.utils.icon.generate.chemical(tint_1, tint_2, tint_3, tint_4, symbol, scale, shift)
	...
	table.insert(icons, {
		icon = apm.lib.icons.path.chemical_flame_1,
		tint = tint_1,
		icon_size = 64,
		scale = scale,
		shift =
	...
	table.insert(icons, {
		icon = apm.lib.icons.path.chemical_flame_2,
		tint = tint_2,
		icon_size = 64,
		scale = scale,
		shift =
	...
	table.insert(icons, {
		icon = apm.lib.icons.path.chemical_flame_3,
		tint = tint_3,
		icon_size = 64,
		scale = scale,
		shift =
	...
	table.insert(icons, {
		icon = apm.lib.icons.path.chemical_flame_4,
		tint = tint_4,
		icon_size = 64,
		scale = scale,
		shift =
...
local function get_tier_icon(level)
	if level == 1 then
		return apm.lib.icons.dynamics.t1
	elseif level == 2 then
		return apm.lib.icons.dynamics.t2
	elseif level == 3 then
		return apm.lib.icons.dynamics.t3
	elseif level == 4 then
		return apm.lib.icons.dynamics.t4
	elseif level == 5 then
		return apm.lib.icons.dynamics.t5
	else
		return nil
	end
end
