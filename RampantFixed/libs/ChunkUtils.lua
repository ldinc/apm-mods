					...
					local alignment = base.alignment
						...
						for i = 1, #enemyBuildings do
							...
							if not buildingHiveTypeLookup[enemyBuilding.name] then
								...
								local hiveType = buildingHiveTypeLookup[enemyBuilding.name]
								counts[hiveType] = counts[hiveType] + 1
							end
						end
...
function chunkUtils.mapScanEnemyChunk(chunk, map)
	...
	local overpopulation_threshold = 40
	local deleteEntity
	--
    ...
    for i=1,#buildings do
		...
		if thisIsRampantEnemy and (not pendingChunks[chunkIndex]) and building.valid then
			if changingEntities and ((chunk.nextMutationTick  or 0) < tick) then
				if buildingHiveTypeLookup[building.name] or (building.type == "turret") then
	...
	if (mutatesCnt > 0) or (pendingEntitiesCnt > 0) then
		chunk.nextMutationTick = tick + 36000
	end
...
function chunkUtils.entityForPassScan(map, entity)
    ...
    for i=1,#overlapArray do
        ...
        if (chunk ~= -1) then
			local chunkIndex = {chunk.x, chunk.y, map.surface.index}
            map.universe.chunkToPassScan[chunkIndex] = true
        end
    end
end
