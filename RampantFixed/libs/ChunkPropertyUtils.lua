...
function chunkPropertyUtils.processNestActiveness(map, chunk)
    ...
    if (nests > 0) then
		...
		elseif (chunk[BASE_DETECTION_PHEROMONE] > universe.no_pollution_attack_threshold) then	-- + !КДА 
            chunkPropertyUtils.setNestActiveness(map, chunk, mMin(activeness + 5, 20))			
        elseif (chunk[BASE_DETECTION_PHEROMONE] > 0) then		-- + !КДА BASE_PHEROMONE
                    ...
                    if (surface.get_pollution(position) > 0) then
                        ...
                        if (surface.get_pollution(position) > 0) then
                            ...
                            if (surface.get_pollution(position) > 0) then
                                ...
                                if (chunk[BASE_DETECTION_PHEROMONE] > universe.raiding_minimum_base_threshold) then		-- + !КДА BASE_PHEROMONE
