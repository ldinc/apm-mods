...
function baseUtils.evolveEntity(entity, map)
	...
	--	game.print("evolve "..entity.name.."-->"..spawnerName)
		local surface = map.surface
		local position = entity.position
		local universe = map.universe
...
function baseUtils.processBase(chunk, map, tick, base)
	...
	if not base.changingEntities and not base.newAlignmentAndSteps and ((base.evolveTick or 0) <= tick) then
    ...
    base.points = base.points + 1		--map.baseIncrement

    if (base.points > BASE_EVOLVE_THRESHOLD * 2) then
        base.points = BASE_EVOLVE_THRESHOLD
    end
...
function baseUtils.rebuildNativeTables(universe, rg)
    ...
    for i=1,10 do
        --evoToTierMapping[#evoToTierMapping+1] = (((i - 1) * 0.1) ^ 0.5) - 0.05	-- + !КДА 2021.11
