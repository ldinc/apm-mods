...
function aiAttackWave.formSettlers(map, chunk)
    ...
    then
		...
		-- + !КДА single nests have priority
		local FreeNeighborChunks = getFreeNeighborChunks(neighborChunks, map)		-- + !КДА number of non-settled chunks
...
function aiAttackWave.formSquads(map, chunk, tick)
	...
	local squadCreated = false
	local finalSquadCost = universe.finalSquadCost
    if (universe.squadCount < (universe.AI_MAX_SQUAD_COUNT*0.7)) and
            ...
            if squadPosition then
                ...
                if (foundUnits > 0) then
                    ...
                    if tick and (map.state == AI_STATE_AGGRESSIVE) then
						-- + !КДА
						map.squadsGenerated = (map.squadsGenerated or 0) + 1	-- + !КДА for this "attack-wave" only
