...
function chunkProcessor.processScanChunks(universe)
    ...
    for chunkIndex in pairs(chunkToPassScan) do
        local x = chunkIndex[1]
        local y = chunkIndex[2]
        local map = universe.maps[chunkIndex[3]]
		if map then
...
function chunkProcessor.processPendingMutations(universe)
	...
	local mutatesCnt = 0	
	local maxMutations = 10
	while entityId and (maxMutations > mutatesCnt) and (processedCnt < 400) do
		local entityValid = true
		local chunkIndex = entityData.chunkIndex
		map = universe.maps[chunkIndex[3]]
		entityValid = entityValid and entityData.valid and entityData.base and map
		if entityValid then
	...
	if pendingEntities_left == 0 then
		...
		--game.print("pendingEntities_left = "..pendingEntities_left)		-- debug
	end
