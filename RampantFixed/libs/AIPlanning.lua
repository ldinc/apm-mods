...
local function planning(map, evolution_factor, tick)
	...
	local thresholdKf = 1 / mMin((1 + map.universe.retribution*0.01), 2.5)
	...
	local newWaveSize = mMin(mMax(attackWaveMaxSize * (evoForMaxWaveSize ^ 1.4), activeRaidNests*0.02), attackWaveMaxSize)		
	----------
	...
	newWaveSize = newWaveSize + mMin(mFloor(universe.retribution*0.3), 50)
	
	if evolution_factor<.02 then			-- let's give the player time to adapt (example: load mod with map ~ 1000 raiding nests and 0% evo)
		if newWaveSize > universe.attackWaveSize then
			universe.attackWaveSize = mMin(universe.attackWaveSize + 0.0001, newWaveSize)
		else
			universe.attackWaveSize = mMax(universe.attackWaveSize - 0.0001, newWaveSize)
		end	
	else
		universe.attackWaveSize = newWaveSize
	end	
    ...
    local points = mMin(evolution_factor, 1)*100 + (activeNestsModified * 2) + activeRaidNests*0.1       	   	  
	...
	-- if (tick%600) == 0 then
		-- game.print("points: +"..points..", map.points = "..map.points..", tick = "..tick)		-- debug
	-- end	
	...
	elseif (universe.aiNocturnalMode and map.surface.darkness < 0.65) 		-- if daytime and aiNocturnalMode...
		...
		if (map.stateTick <= tick) then
			...
			-- if (not universe.aiNocturnalMode) and (universe.retribution < 1) and (map.state == AI_STATE_SIEGE and mapState == AI_STATE_SIEGE) then		-- no more 2+ sieges at row
				-- if universe.raidAIToggle then
					-- map.state = AI_STATE_RAIDING
				-- else
					-- map.state = AI_STATE_AGGRESSIVE
	...
	if mapStateChanged then
		map.squadsGenerated = 0	-- + !КДА 2021.11
...
local function temperamentPlanner(map)
	...
	if universe.attackWaveSize > 0 then
		evoK = balancingK * evoK *  (universe.attackWaveSize - universe.retribution*0.4)/universe.attackWaveSize
	end	
